...
export function Section({ id, title, children }: SectionProps) {
  return (
    <section id={id} className="scroll-mt-24 space-y-4">
      <h2 className="font-pixel text-lg sm:text-xl">
    ...
    <dl className="divide-y divide-border/70 rounded-2xl border border-border">
      {rows.map((row) => (
        <div key={row.name} className="space-y-1.5 px-4 py-3">
          <dt className="flex flex-wrap items-baseline gap-x-3 gap-y-1 font-mono text-[13px]">
          ...
          <dd className="text-[15px] leading-7 text-muted-foreground">{row.description}</dd>
        </div>
      ))}
    </dl>
  );
